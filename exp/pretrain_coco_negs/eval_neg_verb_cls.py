...
def eval_model(model,dataloader,exp_const):
    ...
    for it,data in enumerate(tqdm(dataloader)):
        ...
        neg_verb_loss,log_softmax = compute_neg_verb_loss(
            att_V_o,
            verb_feats,
            valid_verb_mask,
            model.lang_sup_criterion.fw.f_layer)

        pred_verb = torch.argmax(log_softmax,1).float()
        pred_verb = pred_verb*valid_verb_mask[:,0] + -1*(1-valid_verb_mask[:,0])
        correct = (pred_verb==0).sum().item()
        samples = torch.sum(valid_verb_mask[:,0]).item()
        total_correct += correct
        total_samples += samples

        if it%500==0:
            verb_acc = round(100*total_correct / total_samples,2)
            print('Iter:',it,'Verb Acc:',verb_acc)

    verb_acc = total_correct / total_samples
