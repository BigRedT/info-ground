...
def eval_model(model,dataloader,exp_const):
    ...
    for it,data in enumerate(tqdm(dataloader)):
        ...
        num_objects = data['boxes'][0].size(0)
        ...
        src_filename = dataloader.dataset.get_image_path(data['image_id'][0])
        img = skio.imread(src_filename)

        dst_filename = os.path.join(vis_dir,'original.jpg')
        skio.imsave(dst_filename,img)
        col_dict = {
            0: 'Original Image',
            1: html_writer.image_tag('original.jpg'),
            2: data['caption'][0],
        }
        html_writer.add_element(col_dict)

        box_img = img
        num_boxes = len(boxes) #min(15,len(boxes))
        ...
        for i in range(num_tokens):
            ...
            col_dict = {
                ...
                2: [round(a,2) for a in att[i,box_ids[i]]],
            }
            html_writer.add_element(col_dict)
