...
class DetFeatDatasetConstants(Constants):
    def __init__(self,subset):
        ...
        self.verb_tokens_json = os.path.join(
            coco_paths['proc_dir'],
            coco_paths['extracted']['annos']['verb_tokens'][subset])
        self.verb_vocab_json = os.path.join(
            coco_paths['proc_dir'],
            coco_paths['extracted']['annos']['verb_vocab'][subset])
...
class DetFeatDataset(Dataset):
    def __init__(self,const):
        ...
        self.verb_token_ids = io.load_json_object(
            self.const.verb_tokens_json)
    ...
    def __getitem__(self, i):
        ...
        verb_token_ids = self.verb_token_ids[i]['token_ids']
        if len(verb_token_ids)==0:
            to_return['verb_token_ids'] = []
        else:
            select_verb_token_ids = random.choice(verb_token_ids)
            to_return['verb_token_ids'] = \
                np.array(select_verb_token_ids,dtype=np.int32)
