...
from .models.attention_loss import AttentionLoss
...
def compute_lang_guided_obj_obj_att(word_obj_att,word_word_att):
    ...
    # max_att = torch.max(torch.max(att,1)[0],1)[0].unsqueeze(1).unsqueeze(2)
    # min_att = torch.min(torch.min(att,1)[0],1)[0].unsqueeze(1).unsqueeze(2)
...
def eval_model(model,dataloader,exp_const,step):
    ...
    for it,data in enumerate(tqdm(dataloader)):
        ...
        avg_att_loss += (att_loss.item()*batch_size)
...
def main(exp_const,data_const,model_const):
    ...
    model.att_criterion = AttentionLoss()
    if model.const.model_num != -1:
