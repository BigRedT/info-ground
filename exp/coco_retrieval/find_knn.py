...
def create_search_index(object_index,queries,features_f,search_features_f):
    ...
    for i in tqdm(range(num_objects)):
        ...
        for image_id_obj_id in object_index_:
            if image_id_obj_id in object_queries:
                continue
...
def write_html(knn,vis_dir,image_dir):
    ...
    for i in range(num_objects):
        ...
        for j in range(num_queries):
            ...
            query_image_id = knn[i][j]['query_object'][0]
            src_filename = os.path.join(image_dir,query_image_id+'.jpg')
            dst_filename = os.path.join(obj_dir,query_image_id+'.jpg')
            shutil.copyfile(src_filename,dst_filename)
            col_dict = {0: html_writer.image_tag(query_image_id+'.jpg')}
            num_retrieved = len(knn[i][j]['retrieved_caps'])
            for k in range(num_retrieved):
                retrieved_image_id = knn[i][j]['retrieved_objects'][k][0]
                src_filename = os.path.join(image_dir,retrieved_image_id+'.jpg')
                dst_filename = os.path.join(obj_dir,retrieved_image_id+'.jpg')
                shutil.copyfile(src_filename,dst_filename)
                col_dict[1+k] = html_writer.image_tag(retrieved_image_id+'.jpg')
...
def main(**kwargs):
    ...
    write_html(knn,kwargs['vis_dir'],image_dir)
