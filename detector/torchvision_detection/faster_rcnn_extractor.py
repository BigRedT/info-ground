...
class FasterRCNN(GeneralizedRCNN,WritableToFile):
    ...
    def forward(self, images, proposals):
        ...
        Arguments:
            ...
            proposals (list[Tensor]): boxes to extract features form
    ...
    def split_per_image(self,to_split,proposals):
        ...
        for img_idx in range(len(proposals)):
            num_proposals = proposals[img_idx].size(0)
            split.append(to_split[count:count+num_proposals])
            count = count + num_proposals
        return split
